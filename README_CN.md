...
- **局内消息互动** — 新增聊天面板，支持自由发送消息；内置 10 条欢乐斗地主风格快捷语音（如"快点吧，我等到花儿都谢了！"），发送时通过浏览器 TTS 中文播报，不同玩家自动分配不同音色；消息气泡实时弹出在玩家座位旁
- **局内赔率单** — 牌桌内实时账单：自动记录每一局的盲注、跟注、加注、全下、获胜等筹码变动明细（含手数、变动金额、变动后余额），顶部汇总当前余额 / 总投入 / 总盈亏
- **对局金额门槛** — 创建房间时可设置"对局金额（进入门槛）"，账户余额不足的玩家无法加入房间；房主创建时同样校验
- **操作人提示强化** — 当前行动玩家头像新增绿色脉冲光晕（全员可见），轮到自己时弹出"轮到你了，请操作！"横幅并播放提示音
- **后台改余额实时同步** — 管理后台修改账号余额时，若该账号在对局中，对局内筹码立即同步（等待区同样生效），彻底解决"改了余额被对局结算覆盖"的问题
- **移除语音通话** — 下线 LiveKit 语音通话功能（麦克风/扬声器按钮、voice-token 接口及相关依赖已清理）
